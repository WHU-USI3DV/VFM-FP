...
class Attention(nn.Module):
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        q = self.query(input).view(batch_size, -1, height * width).permute(0, 2, 1)#转置
...
class MultiheadAttention(nn.Module):
    def __init__(self, dim_in ,n_heads=4, dropout=0.1) -> None:
        ...
        #维度必须要能被head数量整除
        assert self.hid_dim % n_heads == 0,"必须整除"
    ...
    def forward(self,x):
        ...
        nh=self.n_head
        dk=self.hid_dim//nh#每个head的维度

#--------------------------[ 1, 4, 3, 2]原q的形状为1，4，6,主动分为3瓣
        q=self.query(x)
        q=q.reshape(batch,-1, nh, dk).transpose(1,2)
        #再转变为[1,3,4,2],4，2是每一个head的维度（4为原来的in-dim，2为自己的维度）
        k=self.key(x)
        ...
        dist=self.dropout(dist)#比单头注意力增加了dropout rate

        att=torch.matmul(dist,v)#batch,nh,n,dv
        att=att.transpose(1,2).reshape(batch,-1,self.hid_dim)#又拼起来（batch,n,d-model）

        out=self.concat(att)
        out=out.view(batch,-1, height, width)

        return out+x#没有类似resnet的设计？——添加+x
...
class Danet_PositAttention(nn.Module):
    def __init__(self, in_channels) -> None:
        ...
        self.key   = nn.Conv2d(in_channels, in_channels // 8, kernel_size = 1, stride = 1)
        self.value = nn.Conv2d(in_channels, in_channels, kernel_size = 1, stride = 1)
        self.alpha = nn.Parameter(torch.zeros(1))  #gamma为一个衰减参数，由torch.zero生成，nn.Parameter的作用是将其转化成为可以训练的参数.
        self.softmax = nn.Softmax(dim = -1)
